...
// --- Helper: Parse CSV from Excel (handles semicolons/commas and quotes) ---
...
export default function Inventario({ navigate }) {
  ...
  const filtered = useMemo(() => {
    return items.filter(i => {
      ...
      const matchSearch = !search ||
        ...
        i.codigoInventario.toLowerCase().includes(q) ||
  ...
  const openNew = () => {
    setEditItem(null);
    setForm({ ...EMPTY_FORM, biblioteca: user.biblioteca || BIBLIOTECAS[0] });
    setErrors({});
    setShowModal(true);
  };
  ...
  const handleSave = () => {
    ...
    setTimeout(async () => {
      const record = { ...form, id: editItem ? editItem.id : null };
      await guardarEquipo(record);
  ...
  // Clave única: número de serie (primario) ó código de inventario (secundario)
  ...
  const handleImport = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (parsed.length === 0) {
          setImportError("No se han podido importar los datos. Asegúrate de que no esté vacío y tenga cabeceras.");
        ...
        setTimeout(async () => {
          ...
          if (toAdd.length > 0) {
            await importarEquipos(toAdd);
            addToast(`Importación lista: ${toAdd.length} nuevos equipos añadidos.`, "success");
          }
          if (duplicates > 0) {
            addToast(`${duplicates} equipos ya existían y se han omitido`, "warning");
          }
  ...
  const clearFilters = () => {
    setSearch(""); setTipo("TODOS"); setEstado("TODOS"); setBiblio("TODAS"); setTagFilter("");
  };
  ...
  return (
    <div className="p-8">
      ...
      {importError && (
        <div className="mb-4 bg-red-900/20 border border-red-800/40 rounded-lg px-4 py-3 text-red-400 text-xs">
          {importError}
        </div>
      )}
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 mb-6 space-y-3">
        <div className="flex flex-wrap gap-3">
          <input
            ...
            className="flex-1 min-w-48 bg-zinc-800 border border-zinc-700 rounded px-3 py-2 text-zinc-200 text-sm focus:outline-none focus:border-amber-400/60 transition-colors"
          />
          <select value={biblioFilter} onChange={e => setBiblio(e.target.value)} className="bg-zinc-800 border border-zinc-700 rounded px-3 py-2 text-zinc-200 text-sm focus:outline-none focus:border-amber-400/60 transition-colors">
            <option value="TODAS">Todas las bibliotecas</option>
            {BIBLIOTECAS.map(b => <option key={b}>{b}</option>)}
          </select>
          <select value={tipo} onChange={e => setTipo(e.target.value)} className="bg-zinc-800 border border-zinc-700 rounded px-3 py-2 text-zinc-200 text-sm focus:outline-none focus:border-amber-400/60 transition-colors">
            {TIPOS.map(t => <option key={t}>{t}</option>)}
          </select>
          <select value={estado} onChange={e => setEstado(e.target.value)} className="bg-zinc-800 border border-zinc-700 rounded px-3 py-2 text-zinc-200 text-sm focus:outline-none focus:border-amber-400/60 transition-colors">
            {ESTADOS.map(s => <option key={s}>{s}</option>)}
          </select>
        </div>

        <div className="flex flex-wrap items-center gap-1.5 pt-1">
          <span className="text-zinc-600 text-xs mr-2 uppercase tracking-widest font-bold">FILTRAR POR ETIQUETA:</span>
          <button
          ...
          {(search || tipo !== "TODOS" || estado !== "TODOS" || biblioFilter !== "TODAS" || tagFilter) && (
            <button onClick={clearFilters} className="text-amber-400/60 hover:text-amber-400 text-xs ml-auto transition-colors">Limpiar filtros ✕</button>
          )}
        </div>
      </div>
        ...
        <table className="w-full text-sm table-fixed">
          ...
          <thead>
            <tr className="border-b border-zinc-800">
              {["#", "EQUIPO / CÓDIGO", "TIPO", "BIBLIOTECA", "ESTADO", "ETIQUETAS", ""].map(h => (
                <th key={h} className="text-left text-zinc-500 text-[10px] font-bold tracking-widest uppercase px-3 py-3">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-zinc-800/60">
            {filtered.length === 0 && (
              <tr>
                <td colSpan={7}>
                  <EmptyState title="Inventario vacío" icon="📦" message="No hay equipos registrados o los filtros son demasiado estrictos." />
                </td>
              </tr>
            )}
            {filtered.map((item, idx) => (
              <tr key={item.id} className="hover:bg-zinc-800/40 transition-colors group">
                ...
                <td className="px-3 py-2.5 min-w-0">
                  <div className="text-zinc-200 font-medium truncate">{item.nombre}</div>
                  <div className="text-zinc-600 text-[10px] font-mono truncate">{item.codigoInventario || item.serie || "SIN CÓDIGO"}</div>
                </td>
                <td className="px-3 py-2.5 text-zinc-400 text-xs">{item.tipo}</td>
                <td className="px-3 py-2.5 text-zinc-400 text-xs truncate" title={item.biblioteca}>{item.biblioteca}</td>
                ...
                <td className="px-3 py-2.5">
                  <div className="flex items-center gap-1 justify-end">
                    <button onClick={() => setShowDetail(item)} title="Ver detalle" className="p-1.5 rounded text-zinc-500 hover:text-amber-400 hover:bg-zinc-800 transition-colors text-xs font-mono">◫</button>
                    <button onClick={() => openEdit(item)} title="Editar" className="p-1.5 rounded text-zinc-500 hover:text-amber-400 hover:bg-zinc-800 transition-colors text-xs font-mono">✎</button>
                    {isAdmin && (
                      <button onClick={() => handleDelete(item)} title="Baja" className="p-1.5 rounded text-zinc-600 hover:text-red-400 hover:bg-zinc-800 transition-colors text-xs font-mono">✕</button>
                    )}
                  </div>
                </td>
              </tr>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
              {[
                ...
                <div key={k} className="bg-zinc-800/40 border border-zinc-700/30 rounded p-3">
                  <div className="text-zinc-600 text-[9px] font-bold tracking-widest mb-1 uppercase">{k}</div>
                  <div className="text-zinc-200 text-sm font-medium">{v}</div>
                </div>
              ))}
            </div>
            {(showDetail.etiquetas || []).length > 0 && (
              <div>
                <div className="text-zinc-600 text-[9px] font-bold tracking-widest mb-2 uppercase">ETIQUETAS DEL EQUIPO</div>
            ...
            <div className="flex gap-2 pt-2 border-t border-zinc-800 flex-wrap">
              <Btn size="md" onClick={() => {
                setShowDetail(null);
                navigate("incidencias", null, { _equipoId: showDetail.id, _equipoNombre: showDetail.nombre });
              }}>Ver incidencias del equipo</Btn>
      ...
      {showModal && (
        <Modal title={editItem ? "Editar Dispositivo" : "Registrar nuevo dispositivo"} onClose={() => setShowModal(false)} wide>
          <div className="space-y-5">
            ...
            <div>
              <label className="block text-zinc-400 text-[10px] font-bold tracking-widest uppercase mb-1.5">ETIQUETAS PERSONALIZADAS</label>
      ...
      {itemToDelete && (
        <Modal title="Confirmar baja del equipo" onClose={() => setItemToDelete(null)}>
          <div className="space-y-4 text-center py-2">
            ...
            <p className="text-zinc-300 text-sm">¿Estás seguro de que deseas dar de baja el equipo <strong>{itemToDelete.nombre}</strong>?</p>
            <p className="text-zinc-500 text-xs">El equipo pasará a estado BAJA y se conservará su historial por seguridad.</p>
