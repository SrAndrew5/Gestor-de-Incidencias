...
export default function Incidencias({ navigate, plantillaActiva = null, onPlantillaUsada = null }) {
  ...
  const [nuevo, setNuevo] = useState({
    titulo: "", categoria: "HARDWARE", prioridad: null, descripcion: "", equipoId: null, etiquetas: [], biblioteca: isUsuario ? user.biblioteca : BIBLIOTECAS[0]
  });
  ...
  // Detectar navegación desde Inventario o Plantillas
  useEffect(() => {
    if (plantillaActiva) {
  ...
  const handleSave = () => {
    ...
    setTimeout(async () => {
      const ticket = {
        ...nuevo,
        id: Date.now(), // El backend asignará el ID real, pero lo mantenemos para el mock
  ...
  return (
    <div className="p-8">
      ...
      {importError && <div className="mb-4 bg-red-900/20 border border-red-800/40 rounded-lg px-4 py-3 text-red-400 text-xs">{importError}</div>}

      <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 mb-6 space-y-3">
        <div className="flex flex-wrap gap-3">
          <input type="text" placeholder="Buscar incidencia, biblioteca, creador..." value={search} onChange={e => setSearch(e.target.value)} className="flex-1 min-w-48 bg-zinc-800 border border-zinc-700 rounded px-3 py-2 text-zinc-200 text-sm placeholder-zinc-600 focus:outline-none focus:border-amber-400/60 transition-colors" />
          {!isUsuario && (
          ...
          <select value={prioridad} onChange={e => setPrioridad(e.target.value)} className="bg-zinc-800 border border-zinc-700 rounded px-3 py-2 text-zinc-200 text-sm focus:outline-none focus:border-amber-400/60">
            {PRIORIDADES.map(p => (
          ...
          <select value={categoria} onChange={e => setCategoria(e.target.value)} className="bg-zinc-800 border border-zinc-700 rounded px-3 py-2 text-zinc-200 text-sm focus:outline-none focus:border-amber-400/60">
        ...
        {allTags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 items-center pt-1">
            ...
            <button onClick={() => setTagFilter("")} className={`text-xs px-2 py-0.5 rounded transition-colors ${!tagFilter ? "bg-zinc-700 text-zinc-200" : "text-zinc-500 hover:text-zinc-300"}`}>todas</button>
            {allTags.map(tag => (
        ...
        {(hasFilters || equipoFilterId) && (
          <div className="flex flex-wrap items-center gap-2 p-3 bg-zinc-900 border border-zinc-800 rounded-lg">
            <span className="text-zinc-600 text-xs">Filtros activos:</span>
            {equipoFilterId && <span className="bg-amber-400/10 text-amber-400 text-xs px-2 py-0.5 rounded border border-amber-400/20">Equipo: {equipoFilterNombre}</span>}
            ...
            <button onClick={clearFilters} className="text-zinc-500 hover:text-zinc-300 text-xs ml-auto transition-colors">Limpiar ✕</button>
          </div>
        )}
      </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
        <table className="w-full text-sm table-fixed">
          ...
          <tbody className="divide-y divide-zinc-800/60">
            ...
            {filtered.map(inc => (
              <tr key={inc.id} onClick={() => navigate("detalle", inc.id)} className="hover:bg-zinc-800/50 cursor-pointer transition-colors border-l-2 border-l-transparent">
                <td className="px-3 py-2.5 text-zinc-600 font-mono">#{inc.id}</td>
                <td className="px-3 py-2.5 min-w-0">
                  <div className="text-zinc-200 font-medium truncate">{inc.titulo}</div>
                  <div className="text-zinc-600 text-xs truncate">{inc.biblioteca} · {inc.creadoPor} · {inc.fecha}</div>
                </td>
                <td className="px-3 py-2.5">
                  <span className="text-xs bg-zinc-800 text-zinc-400 px-1.5 py-0.5 rounded">{inc.categoria}</span>
                </td>
                <td className="px-3 py-2.5">
                ...
                <td className="px-3 py-2.5 text-zinc-400 text-xs truncate">{inc.asignado || "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {showModal && (
        <Modal title="Nueva incidencia" onClose={() => setShowModal(false)} wide>
          <div className="space-y-4">
            {plantillaNombre && (
              <div className="bg-amber-400/10 border border-amber-400/20 text-amber-500 p-3 rounded text-xs flex justify-between items-center">
                <span>Plantilla activa: <strong>{plantillaNombre}</strong></span>
                <button onClick={() => setPlantillaNombre("")} className="hover:text-amber-300">✕</button>
              </div>
            )}
            <Input 
               label="TÍTULO" 
            ...
            />
            <div>
              <label className="block text-zinc-400 text-[10px] font-bold tracking-widest uppercase mb-1.5">
                DESCRIPCIÓN <span className="text-red-500">*</span>
              </label>
              <textarea 
                value={nuevo.descripcion} 
                onChange={e => { setNuevo(p => ({ ...p, descripcion: e.target.value })); if(errors.descripcion) setErrors(p=>({...p, descripcion:null})); }} 
                className={`w-full bg-zinc-800 border ${errors.descripcion ? 'border-red-500/50 focus:border-red-500' : 'border-zinc-700 focus:border-amber-400/60'} rounded px-3 py-2 text-zinc-200 text-sm focus:outline-none transition-all`} 
                rows={4} 
                placeholder="Detalles..." 
              />
              {errors.descripcion && <p className="text-red-400 text-[10px] font-medium mt-1">{errors.descripcion}</p>}
            </div>
